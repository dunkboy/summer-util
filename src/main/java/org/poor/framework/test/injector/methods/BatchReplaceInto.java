 ...
 * +---------------------------+
 * <p>Title: BatchReplaceInto</p>
 * <p>Description: replace into批量插入适用于mysql</p>
 ...
 * @version 1.0
 **/
public class BatchReplaceInto extends AbstractMethod
{
    @Override
    public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo)
    {
        KeyGenerator keyGenerator = new NoKeyGenerator();
        CustomSqlMethod sqlMethod = CustomSqlMethod.BATCH_REPLACE_INTO;
    ...
    public Map<String, List<String>> doBatchInsertField(Class<?> modelClass)
    {
        ...
        List<Field> batchInsertField = fields.stream().filter(item -> item.getAnnotation(Column.class) != null)
                .filter(item -> item.getAnnotation(Column.class).batchReplaceInto())
